...
def upload_video():
    ...
    if request.method == 'POST':
        file = request.files.get('video_file')  # Using .get() to avoid KeyError
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                # Placeholder for gait analysis function (replace with actual detection)
                

                # Log the video upload and result
                cursor.execute(
                    "INSERT INTO logs (physiotherapist_id,patient_id, filepath, result) VALUES (%s, %s,%s, %s)",
                    (physiotherapist_id,patient_id, filepath, result)
                )
                connection.commit()
                ...
                flash('Error occurred during video upload.', 'danger')
            ...
            flash('Invalid file format. Only mp4, avi, mov, and mkv are allowed.', 'danger')
...
def process_video(video_path):
    global no_total_frame, no_neck_frame, no_rightleg_frame, no_leftleg_frame, no_limping_right, no_limping_left, no_swing_right, no_swing_left, lead_foot_right, lead_foot_left
    pose = mp_pose.Pose()
    cap = cv2.VideoCapture(video_path)

    # Add your video processing code here (the code you provided)
    ...
    while True:
        try:
            ...
            limp_right_percentage = (no_limping_right / no_total_frame) * 100
            limp_left_percentage = (no_limping_left / no_total_frame) * 100
            neck_percentage = (no_neck_frame / no_total_frame) * 100
            right_leg_percentage = (no_rightleg_frame / no_total_frame) * 100
            left_leg_percentage = (no_leftleg_frame / no_total_frame) * 100
            swing_right_percentage = (no_swing_right / no_total_frame) * 100
            swing_left_percentage = (no_swing_left / no_total_frame) * 100

            if limp_right_percentage > 75 or limp_left_percentage > 75:
                no_gait += 1
                result = f"\n\t- Limping: {max(limp_right_percentage, limp_left_percentage):.2f}%"
                results.append(result)  # Store result for file
                print(result, end='')

            if neck_percentage > 60:
                no_gait += 1
                result = f"\n\t- Slouch posture: {neck_percentage:.2f}%"
                results.append(result)  # Store result for file
                print(result, end='')

            if right_leg_percentage > 99 or left_leg_percentage > 99:
                no_gait += 1
                result = f"\n\t- Circumduction: {max(right_leg_percentage, left_leg_percentage):.2f}%"
                results.append(result)  # Store result for file
                print(result, end='')

            if swing_right_percentage > 90:
                no_gait += 1
                result = f"\n\t- Right arm no swing: {swing_right_percentage:.2f}%"
                results.append(result)  # Store result for file
                print(result, end='')

            if swing_left_percentage > 90:
                no_gait += 1
                result = f"\n\t- Left arm no swing: {swing_left_percentage:.2f}%"
                results.append(result)  # Store result for file
                print(result, end='')
            ...
            with open("gait_analysis_results.txt", "w") as file:
                file.write("\n".join(results))  # Write results to the file
            return result
            break
    ...
    # Optionally, you can save the processed video or any output you wish
